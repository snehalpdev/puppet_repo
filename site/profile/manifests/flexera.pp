...
class profile::flexera (
  ...
  if ($install_flexera == 'true') {
    ...
    exec { 'FlexNet Inventory Agent':
      command   => "${extract_dir}\\FlexNetInventoryAgent.msi /qn TRANSFORMS=InstallFlexNetInvAgent.mst BOOTSTRAPSCHEDULE='Bootstrap Machine Schedule' GENERATEINVENTORY='true' APPLYPOLICY='true'",
