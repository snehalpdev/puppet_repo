...
class profile::flexera (
  ...
  if ($install_flexera == 'true') {
    ...
    exec { 'FlexNet Inventory Agent':
      command  => "'c:\windows\system32\msiexec.exe' /i 'C:\temp\fnms\extract\FlexNet Inventory Agent.msi' /qn TRANSFORMS=InstallFlexNetInvAgent.mst BOOTSTRAPSCHEDULE='Bootstrap Machine Schedule' GENERATEINVENTORY='true' APPLYPOLICY='true'",
      provider => powershell,
